...
Then install this candidate using the exact command in **Install This Release** below. Use `composer remove --no-update` without `--dev` if the old package was in `require` rather than `require-dev`. Existing lock files do not migrate automatically. After installation, rebuild cleanly:
