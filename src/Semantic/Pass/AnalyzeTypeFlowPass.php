...
final class AnalyzeTypeFlowPass implements SemanticPass
{
    ...
    private function analyzeArrowFunction(Expr\ArrowFunction $function, Scope $outer, FlowState $outerState, ?ClassSymbol $class): void
    {
        ...
        if ($this->compatibility->compare($declared, $actual->type, $this->context->symbols, $function->expr instanceof Expr\Array_) === TypeCompatibilityResult::Incompatible) {
            $this->addDiagnostic(
    ...
    private function validateCall(
        ...
        foreach ($binding->arguments as $bound) {
            ...
            if ($this->compatibility->compare($expected, $actual, $this->context->symbols, $bound->argument->value instanceof Expr\Array_) === TypeCompatibilityResult::Incompatible) {
                $related = $contract->origin === CallableOrigin::IntrinsicOverride
                    ? []
    ...
    private function analyzeAssignmentTarget(
        ...
        if ($target instanceof Expr\ArrayDimFetch) {
            ...
            if ($propertyContainer && $container instanceof TypedArrayType) {
                if ($this->compatibility->compare($container->valueType, $actual, $this->context->symbols, $value instanceof Expr\Array_) === TypeCompatibilityResult::Incompatible) {
                    $this->addDiagnostic(
        ...
        if ((($target instanceof Expr\PropertyFetch || $target instanceof Expr\NullsafePropertyFetch)
            ...
            foreach ($resolution->targets as $resolvedTarget) {
                ...
                if ($expected !== null) {
                    ...
                    if ($this->compatibility->compare($expected, $actual, $this->context->symbols, $value instanceof Expr\Array_) === TypeCompatibilityResult::Incompatible) {
                        $this->addDiagnostic(
    ...
    private function validateReturn(Stmt\Return_ $return, ?ExpressionTypeResolution $actual, ?Type $declared): void
    {
        ...
        if ($isVoid || $isNever
            || $this->compatibility->compare($declared, $actual->type, $this->context->symbols, $return->expr instanceof Expr\Array_) === TypeCompatibilityResult::Incompatible) {
            $this->addDiagnostic(
