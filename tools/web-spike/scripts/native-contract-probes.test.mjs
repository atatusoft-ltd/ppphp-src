...
const accepted = () => ({ suite: 'native-contract', done: true, cases: nativeContractProbes.map(probe => ({
  id: probe.id, computeStarted: true, semantics: 'PASS', kind: probe.entropyDenied ? 'trap' : 'completed',
  stdout: 'ok', stderr: '', exitCode: probe.exitCode, sideEffect: false,
  ...(probe.entropyDenied ? { entropyReads: 1, error: 'BP-7R entropy unavailable' } : {}),
})) });
...
test('native qualification rejects missing execution, failed processes and fabricated entropy evidence', () => {
  ...
  for (const change of [
    ...
    data => { data.cases.find(item => item.id === 'native-entropy-failure').error = 'unrelated trap'; },
